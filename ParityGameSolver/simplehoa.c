...
// Function to print BTree node as dot
void bTreeToDot(FILE* fp, BTree* node) {
    if (node == NULL)
        return;
    switch (node->type) {
        case NT_AP:
            ...
            break;
        case NT_ALIAS:
            fprintf(fp, "@%s", node->alias);
            break;
        case NT_AND:
            fprintf(fp, "(");
            bTreeToDot(fp, node->left);
            fprintf(fp, " & ");
            bTreeToDot(fp, node->right);
            fprintf(fp, ")");
            break;
        case NT_OR:
            fprintf(fp, "(");
...
// Main function to generate dot-file content
void generateDotFile(HoaData* hoa_data, const char* filename) {
    ...
    fprintf(fp, "digraph OmegaAutomaton {\nrankdir=LR;\nnodesep=1; ranksep=1.5;\n");

    for (int i = 0; i < hoa_data->noStates; i++) {
        State* state = &hoa_data->states[i];
        
        fprintf(fp, "%d [label=\"%d", state->id, state->id);
        if (state->name != NULL) {
            fprintf(fp, " %s", state->name);
        }
        if (state->label != NULL) {
            fprintf(fp, " (");
            bTreeToDot(fp, state->label);
            fprintf(fp, ")");
        }
        if (state->accSig != NULL) {
            fprintf(fp, "\\n acc sets: ");
            for (int a = 0; a < state->noAccSig; a++)
                fprintf(fp, "%d, ", state->accSig[a]);
        }
        fprintf(fp, "\"];\n");
        ...
        for (int j = 0; j < state->noTrans; j++) {
            ...
            for (int k = 0; k < trans->noSucc; k++) {
                fprintf(fp, "%d -> %d", state->id, trans->successors[k]);
                if(trans->label != NULL || trans->accSig != NULL) {
                    fprintf(fp, " [label=\"");
                    if(trans->label != NULL) {
                        bTreeToDot(fp, trans->label);
                    }
                    if(trans->accSig != NULL) {
                        fprintf(fp, "\\nAcc Sets: ");
                        for (int a = 0; a < trans->noAccSig; a++)
                            fprintf(fp, "%d%s", trans->accSig[a], i == trans->noAccSig - 1 ? "" : ", ");
                    }
                }
                fprintf(fp, "\"];\n");
            }
    ...
    for (int i = 0; i < hoa_data->noStart; i++) {
        fprintf(fp, "init%d[shape=none, label=\"\"]\ninit%d -> %d [minlen=0.75,weight=100,constraint=false];\n", i, i, hoa_data->start[i]);
    }

    // Print APs
    fprintf(fp, "props [shape=record,margin=0.2,label=\"APs: ");
    for (int i = 0; i < hoa_data->noAPs; i++) {
        fprintf(fp, "%s%s", hoa_data->aps[i], i == hoa_data->noAPs - 1 ? "" : ", ");
    }
    fprintf(fp, "\\nControllable: ");
    for (int i = 0; i < hoa_data->noCntAPs; i++) {
        fprintf(fp, "%d%s", hoa_data->cntAPs[i], i == hoa_data->noCntAPs - 1 ? "" : ", ");
    }

    // Print acceptance
    fprintf(fp, "|Acceptance: ");
    bTreeToDot(fp, hoa_data->acc);
    fprintf(fp, "\\nAcceptance Parameters: ");
    for (int i = 0; i < hoa_data->noANPs; i++) {
        fprintf(fp, "%s%s", hoa_data->accNameParameters[i], i == hoa_data->noANPs - 1 ? "" : ", ");
    }

    // Print Properties
    fprintf(fp, "|Properties: ");
    for (int i = 0; i < hoa_data->noProps; i++) {
        fprintf(fp, "%s%s", hoa_data->properties[i], i == hoa_data->noProps - 1 ? "" : ", ");
    }
    fprintf(fp, "\"];\n}");
